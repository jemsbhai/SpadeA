...
function DamagePopup({ damage, side }: { damage: number; side: "left" | "right" }) {
  ...
  return (
    <motion.span
      ...
      className={`absolute top-0 text-sm font-black pointer-events-none ${side === "left" ? "left-2" : "right-2"}`}
...
export function HealthBar({ health, lastDamageToAttorney = 0, lastDamageToDefendant = 0 }: HealthBarProps) {
  ...
  useEffect(() => {
    if (lastDamageToAttorney > 0 || lastDamageToDefendant > 0) setDamageKey((k) => k + 1);
    ...
    <div className="flex items-center gap-3 px-5 py-2.5 shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
      <div className="flex-1 relative">
        ...
        <div className="h-2 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
          <motion.div className="h-full rounded-full" style={{ background: attColor, originX: 0 }} animate={{ width: `${attPct}%` }} transition={{ duration: 0.6, ease: "easeOut" }} />
        </div>
        <AnimatePresence mode="popLayout">
          {lastDamageToAttorney > 0 && <DamagePopup key={`att-${damageKey}`} damage={lastDamageToAttorney} side="left" />}
        </AnimatePresence>
      </div>
      <div className="flex flex-col items-center shrink-0 px-1">
        <span className="text-[9px] font-semibold tracking-widest" style={{ color: "var(--text-muted)" }}>VS</span>
      </div>
      <div className="flex-1 relative">
        <div className="flex items-center justify-between mb-1">
        ...
        <div className="h-2 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
          <motion.div className="h-full rounded-full ml-auto" style={{ background: defColor, originX: 1 }} animate={{ width: `${defPct}%` }} transition={{ duration: 0.6, ease: "easeOut" }} />
        </div>
        <AnimatePresence mode="popLayout">
          {lastDamageToDefendant > 0 && <DamagePopup key={`def-${damageKey}`} damage={lastDamageToDefendant} side="right" />}
        </AnimatePresence>
      </div>
    </div>
  );
