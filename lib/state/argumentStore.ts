...
export const useArgumentStore = create<ArgumentState & ArgumentActions>((set, get) => ({
  ...
  initCase: (caseData: CaseData) =>
    set({
      ...
      analysis: {
        ...createInitialAnalysis(),
        attorneyScore: { validPoints: caseData.attorney_points.length, fallacies: 0, challenged: 0 },
        defendantScore: { validPoints: caseData.defendant_points.length, fallacies: 0, challenged: 0 },
      },
      health: { ...DEFAULT_HEALTH },
      messages: [],
      suggestions: [],
  ...
  updateAnalysis: (fallacies: FallacyEntry[], assumptions: AssumptionEntry[]) =>
    set((state) => {
      ...
      if (fallacies.length > 0) analysis = applyFallacies(analysis, fallacies, state.exchangeCount);
      if (assumptions.length > 0) analysis = applyAssumptions(analysis, assumptions);
      return { analysis };
    }),
  ...
  applyDamage: (toAttorney: number, toDefendant: number) => {
    ...
    set({ health: { ...state.health, attorneyHP: newAttorneyHP, defendantHP: newDefendantHP } });
  ...
  useEvidenceCard: (id: string) => {
    set((state) => ({
      evidenceCards: state.evidenceCards.map((c) => (c.id === id ? { ...c, used: true } : c)),
    }));
  },
  ...
  persist: () => {
    try {
      ...
      const data: ArgumentState = {
        phase: state.phase, caseData: state.caseData, attorneyPoints: state.attorneyPoints,
        defendantPoints: state.defendantPoints, analysis: state.analysis, health: state.health,
        messages: state.messages, suggestions: state.suggestions, evidenceCards: state.evidenceCards,
        exchangeCount: state.exchangeCount, isGeneratingCase: false, isAttorneyThinking: false,
        isSuggestionsLoading: false, outcome: state.outcome,
      };
  ...
  hydrate: () => {
    try {
      ...
      if (raw) {
        ...
        set({ ...data, health: data.health ?? { ...DEFAULT_HEALTH }, isGeneratingCase: false, isAttorneyThinking: false, isSuggestionsLoading: false });
      }
    } catch { /* ignore */ }
  },
}));
