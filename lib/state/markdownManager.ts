...
export function generatePointsMd(points: CasePoint[], side: 'prosecution' | 'defense'): string {
  const title = side === 'prosecution' ? 'Prosecution Arguments' : 'Defense Arguments';
  const rows = points.map((p) => `| ${p.id} | ${p.claim} | ${p.evidence} | ${p.status} |`).join('\n');
  const concessions = points.filter((p) => p.status === 'refuted').map((p) => `- ${p.id}: ${p.claim}`).join('\n');
  return `# ${title}\n\n| ID | Claim | Evidence | Status |\n|----|-------|----------|--------|\n${rows}\n\n## Concessions\n${concessions || '- None'}\n`;
}

export function generateAnalysisMd(analysis: AnalysisState): string {
  const assumptionRows = analysis.assumptions.map((a) => `| ${a.side} | ${a.assumption} | ${a.state} |`).join('\n');
  const fallacyRows = analysis.fallacies.map((f) => `| ${f.side} | ${f.type} | ${f.context} | ${f.exchangeNumber} |`).join('\n');
...
export function applyPointUpdates(
  ...
  for (const update of updates) {
    ...
    if (aIdx >= 0) { newAttorney[aIdx] = { ...newAttorney[aIdx], status: update.new_status }; continue; }
    const dIdx = newDefendant.findIndex((p) => p.id === update.id);
    if (dIdx >= 0) { newDefendant[dIdx] = { ...newDefendant[dIdx], status: update.new_status }; }
  }
  return { attorneyPoints: newAttorney, defendantPoints: newDefendant };
}

export function applyFallacies(analysis: AnalysisState, fallacies: FallacyEntry[], exchangeNumber: number): AnalysisState {
  const newFallacies = [...analysis.fallacies, ...fallacies.map((f) => ({ ...f, exchangeNumber }))];
  return {
    ...analysis,
    fallacies: newFallacies,
    attorneyScore: { ...analysis.attorneyScore, fallacies: newFallacies.filter((f) => f.side === 'attorney').length },
    defendantScore: { ...analysis.defendantScore, fallacies: newFallacies.filter((f) => f.side === 'defendant').length },
  };
}

export function applyAssumptions(analysis: AnalysisState, assumptions: AssumptionEntry[]): AnalysisState {
  const newAssumptions = [...analysis.assumptions];
  for (const update of assumptions) {
    const idx = newAssumptions.findIndex((a) => a.side === update.side && a.assumption === update.assumption);
    if (idx >= 0) { newAssumptions[idx] = { ...newAssumptions[idx], state: update.new_state }; }
    else { newAssumptions.push({ side: update.side, assumption: update.assumption, state: update.new_state }); }
  }
  return { ...analysis, assumptions: newAssumptions };
}

export function recalcScores(analysis: AnalysisState, attorneyPoints: CasePoint[], defendantPoints: CasePoint[]): AnalysisState {
  return {
    ...analysis,
    attorneyScore: {
      ...analysis.attorneyScore,
      validPoints: attorneyPoints.filter((p) => p.status === 'unchallenged' || p.status === 'proven').length,
      challenged: attorneyPoints.filter((p) => p.status === 'challenged').length,
    },
    defendantScore: {
      ...analysis.defendantScore,
      validPoints: defendantPoints.filter((p) => p.status === 'unchallenged' || p.status === 'proven').length,
