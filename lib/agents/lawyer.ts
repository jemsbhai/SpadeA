import type { LawyerResponse } from '@/lib/types';
import { callLawyer } from './agentRouter';

export async function getLawyerResponse(userMessage: string, surrender: boolean = false): Promise<LawyerResponse> {
  const json = await callLawyer(userMessage, surrender);
  const data = JSON.parse(json) as LawyerResponse;
  if (!data.message) throw new Error('Invalid lawyer response: missing message');
  return {
